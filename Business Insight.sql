CREATE DATABASE data_analyst_project
 ...
 -- Delete null values 
 DELETE FROM customer_behaviour_analysis
...
SELECT * FROM customer_behaviour_analysis
 ...
 --? Business Problems :- Discounts may reduce profit increasing sales.


 --  Impact :-
          --Identity effectiveness of discounts
          --Reduce unecessary discounts costs
          --Improve profit margins

-------------------------------------------------------------------------------------------------------------------------------------
--3. What is the total revenue generated by males and females customer?
-------------------------------------------------------------------------------------------------------------------------------------
...
SELECT 
 ...
 ORDER BY highest_revenue DESC;
  ...
  --? Business Problems :- Lack of inderstanding of revenue contribution by gender segments.
...
WHERE discount_applied='Yes' AND  purchase_amount > (SELECT AVG(purchase_amount) FROM customer_behaviour_analysis)
 ...
 --? Business Problems :- Company cannot identity high-spending customers who are also discounts users.

 --  Impact :-
          --Identitfy premium discount-sensitive customers
          --Enables targeted discount campaigns
          --Improves customers retention and revenue
...
ORDER BY Avg_ratings ASC
 ...
 --? Business Problems :- No visibility into product performance based on cutomer satisfactions.

 --  Impact :-
          --Promotes high-performing products
          --Improves low-rated products
          --EEnhances customer experience
...
SELECT 
     ...
     ROUND(AVG(purchase_amount),2) as avg_purchased,
...
--? Business Problems :- The effectiveness of subscriptions programs is unknown.

-- Impact 
        --Validates subscription model performance
        --improves customer loyality programs
...
-------------------------------------------------------------------------------------------------------------------------------------
--9. Segment customer into new, returning and loyal based on thier total number of previous purchase,
...
-- Impact :-
        --Iproves product placement & recommendations
...
--Impact :-
       ...
       --Increases convertion to paid programs
...
-- ? Business Problem :- No visiblity into which age group contribution most to revenue.
